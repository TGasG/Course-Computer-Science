...
<section>
    <div class="header">
        ...
        <div class="headerImage">
            <img src="<?= isset($user) && $user['role'] === 'mentor' ? '/img/home-mentor.svg' : '/img/home-user.svg' ?>" alt="home icon">
        </div>
    </div>
    <?php if (isset($user) && $user['role'] === 'mentor') : ?>
        <div class="rekomendasiKelas">
            <div class="titleRK">
                ...
                <svg class="mb-5" width="184" height="7" viewBox="0 0 184 7" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="rekomendasiKelas">
            ...
            <div class="parent-card-mentor d-flex flex-column align-items-center justify-content-center">
                <a href="<?= base_url('/course/add') ?>" class="tambah-course d-flex flex-column align-items-center justify-content-center text-decoration-none text-black">
                        ...
                        <div class="cardCourseText">
                            ...
                            <?php if (isset($user)) : ?>
                                <?php if ($course['isRegistered']) : ?>
                                    <button id="daftar" class="btn-outline-primary btn daftar-belajar" onclick="window.location.href = '<?= base_url('/course/' . $course['id']) ?>'">
                                        Belajar
                                    </button>
                                <?php else : ?>
                                    <form action="<?= base_url('/register/new') ?>" method="post">
                                        ...
                                        <input type="hidden" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>" />
                                        <button class="btn-outline-primary btn daftar-belajar" type="submit" id="daftar">Daftar
                                        </button>
                                    </form>
                                <?php endif ?>
                            <?php else : ?>
                                <button id="daftar" class="btn-outline-primary btn daftar-belajar" onclick="window.location.href = '<?= base_url('/user/login') ?>'">
                                    Daftar
                                </button>
                            <?php endif ?>
                        </div>
